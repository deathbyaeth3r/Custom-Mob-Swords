...
public class DragonForceEntity extends ProjectileItemEntity {
	...
	public void tick() {
        ...
        if (this.world.isRemote) {
           this.world.addParticle(ParticleTypes.DRAGON_BREATH, this.getPosX() - vec3d1.x, this.getPosY() - vec3d1.y + 0.15D, this.getPosZ() - vec3d1.z, 0.0D, 0.0D, 0.0D);
        }
	}
	...
	protected void onImpact(RayTraceResult result) {
		 ...
		 if (result.getType() == RayTraceResult.Type.ENTITY) {
	         ...
	         entity.attackEntityFrom(DamageSource.causeIndirectDamage(this, this.owner).setProjectile(), 4.0F);
	           ...
	           ((LivingEntity)entity).addPotionEffect(new EffectInstance(Effects.WITHER, 15));
	           ((LivingEntity)entity).addPotionEffect(new EffectInstance(Effects.WEAKNESS, 15));
	           entity.setFire(7);
