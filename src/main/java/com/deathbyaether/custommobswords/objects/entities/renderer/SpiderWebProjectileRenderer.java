...
public class SpiderWebProjectileRenderer extends EntityRenderer<SpiderWebProjectileEntity>{
	
	public static final ResourceLocation SPIDERWEB_ENTITY = new ResourceLocation(Main.MOD_ID, "");
    private static final RenderType field_229044_e_ = RenderType.getEntityCutoutNoCull(SPIDERWEB_ENTITY);
    public SpiderWebProjectileRenderer(EntityRendererManager renderManagerIn) {
        super(renderManagerIn);
    }
    
    protected int getBlockLight(SpiderWebProjectileRenderer entityIn, float partialTicks) {
        return 15;
     }

     public void render(SpiderWebProjectileEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        matrixStackIn.push();
	...
	@Override
	public ResourceLocation getEntityTexture(SpiderWebProjectileEntity entity) {
		
		return SPIDERWEB_ENTITY;
	}
