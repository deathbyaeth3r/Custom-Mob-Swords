...
public class SquidInkRenderer extends EntityRenderer<SquidInkEntity> {
    ...
    private static final RenderType RENDER_TYPE = RenderType.entityCutoutNoCull(SQUIDINK_ENTITY);
    public SquidInkRenderer(EntityRendererManager renderManagerIn) {
        super(renderManagerIn);
    }
    
    protected int getBlockLight(SquidInkRenderer entityIn, float partialTicks) {
        return 15;
     }

     public void render(SquidInkEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
	...
	@Override
	public ResourceLocation getTextureLocation(SquidInkEntity entity) {
		
		return SQUIDINK_ENTITY;
	}
